...
- Supervisor or Docker for queue workers (`queue` + `queue-heavy`), scheduler, and Reverb
...
php artisan queue:work redis --queue=heavy --timeout=180
...
Docker Compose runs the `scheduler` service. Native installs still need host cron: `* * * * * php artisan schedule:run`
