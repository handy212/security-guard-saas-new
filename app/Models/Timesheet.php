...
class Timesheet extends Model { use BelongsToTenant; protected $fillable=['tenant_id','guard_id','period_start','period_end','regular_hours','overtime_hours','gross_pay','status','approved_by_user_id','approved_at']; protected function casts(): array { return ['period_start'=>'date','period_end'=>'date','approved_at'=>'datetime']; } public function assignedGuard(): BelongsTo { return $this->belongsTo(Guard::class); } }
