            ...
            <tbody>
                @forelse ($logs as $log)
                    <tr class="table-row-hover" wire:key="audit-{{ $log->id }}">
                        <x-table.td mono>{{ $log->created_at?->format('M j, H:i') }}</x-table.td>
                        <x-table.td>{{ $log->user?->name ?? 'System' }}</x-table.td>
                        <x-table.td mono>{{ $log->action }}</x-table.td>
                ...
                @empty
                    <x-table.empty colspan="4">
                        <x-empty-state compact :title="$hasActiveFilters ? 'No matching events' : 'No audit events yet'" />
                    </x-table.empty>
                @endforelse
            </tbody>
        </x-data-table>
