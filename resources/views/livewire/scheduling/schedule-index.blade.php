<div>
    <x-page-shell title="Schedule" description="Create shifts, assign guards, and manage daily deployments.">
        ...
        <div class="stat-grid">
            <x-stat-card compact label="Shifts today" :value="$stats['shifts_today']" icon="schedules" :href="route('schedules.index', ['date' => $date])" />
            <x-stat-card compact label="Open shifts" :value="$stats['open_shifts']" icon="pause" :tone="$stats['open_shifts'] ? 'warning' : 'success'" :href="route('schedules.open-shifts')" />
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <div class="flex shrink-0 items-start gap-2">
                            ...
                            <x-row-menu>
                                ...
                                <x-row-menu-item wire:click="cancelShift({{ $shift->id }})" wire:confirm="Cancel this shift?">Cancel shift</x-row-menu-item>
                            </x-row-menu>
                        </div>
                    </div>
        ...
        <x-drawer :title="$editingShiftId ? 'Edit shift' : 'Create shift'" width="lg">
            <x-drawer-form wire:submit.prevent="save" :submit-label="$editingShiftId ? 'Save changes' : 'Create shift'">
                ...
                <x-select wire:model="form.status" label="Status">
                    @foreach ($shiftStatuses as $value => $label)
