...
<head>
    ...
    <title>Field — {{ config('app.name', 'GuardCore Pro') }}</title>
...
<header class="sticky top-0 z-20 border-b border-zinc-800 bg-zinc-950/95 px-4 py-3 backdrop-blur">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-2.5">
            <x-brand-mark />
            <div>
                <div class="text-sm font-semibold tracking-tight">GuardCore Pro Field</div>
                <div class="text-[10px] text-zinc-500" id="guard-connection-status">Online</div>
            </div>
        </div>
        <a href="{{ route('dashboard') }}" class="rounded-md px-2 py-1 text-xs text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200">Admin</a>
    </div>
</header>
<main class="mx-auto max-w-lg p-4 pb-28" data-guard-app>{{ $slot }}</main>

<nav class="fixed inset-x-0 bottom-0 z-30 border-t border-zinc-800 bg-zinc-950/95 backdrop-blur" aria-label="Field navigation">
    <div class="mx-auto grid max-w-lg grid-cols-3 text-center text-[11px]">
        <a href="#assignments" class="flex flex-col items-center gap-1 px-2 py-3 text-zinc-400 hover:text-white">
        ...
        <a href="#patrol" class="flex flex-col items-center gap-1 px-2 py-3 text-zinc-400 hover:text-white">
        ...
        <a href="#scan" class="flex flex-col items-center gap-1 px-2 py-3 text-zinc-400 hover:text-white">
