...
<header class="border-b border-zinc-200/80 bg-white dark:border-zinc-800 dark:bg-zinc-900">
    <div class="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
        <div class="flex min-w-0 items-center gap-2.5">
            <x-brand-mark />
            <div class="min-w-0">
                <div class="truncate text-sm font-semibold tracking-tight dark:text-zinc-100">{{ $tenantBranding['name'] ?? $portalTenantName ?? 'Client Portal' }}</div>
                <div class="truncate text-[11px] text-zinc-500 dark:text-zinc-400">{{ $tenantBranding['tagline'] ?? 'Proof of service' }}</div>
            </div>
        </div>
        <button type="button" @click="navOpen = !navOpen" class="rounded-md p-2 text-zinc-600 hover:bg-zinc-100 sm:hidden dark:hover:bg-zinc-800" aria-label="Menu">
        ...
        <nav class="hidden items-center gap-1 text-sm sm:flex">
            ...
            <a href="{{ route('client-portal.invoices') }}" @class([
                'rounded-md px-2.5 py-1.5 font-medium transition',
            ...
            <a href="{{ route('client-portal.approvals') }}" @class([
                'rounded-md px-2.5 py-1.5 font-medium transition',
    ...
    <nav x-show="navOpen" x-cloak class="border-t border-zinc-100 px-4 py-2 sm:hidden dark:border-zinc-800">
        ...
        <a href="{{ route('client-portal.invoices') }}" class="block rounded-md px-2 py-2 text-sm font-medium">Invoices</a>
...
<main class="mx-auto max-w-6xl px-4 py-5 sm:px-6">{{ $slot }}</main>
@livewireScripts
</body>
</html>
