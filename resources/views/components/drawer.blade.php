...
<div
    ...
    role="dialog"
    aria-modal="true"
    aria-labelledby="drawer-title-{{ md5($title) }}"
    @keydown.escape.window="$wire.{{ $closeMethod }}()"
    x-data
    x-init="$nextTick(() => $el.querySelector('[data-drawer-close]')?.focus())"
>
    ...
    <div class="relative flex h-full w-full {{ $widthClass }} flex-col border-l border-zinc-200 bg-white shadow-2xl">
        <div class="flex items-start justify-between border-b border-zinc-100 px-5 py-4">
            <div>
                <h2 id="drawer-title-{{ md5($title) }}" class="text-base font-semibold text-zinc-900">{{ $title }}</h2>
                @if ($description)
                    <p class="mt-0.5 text-sm text-zinc-500">{{ $description }}</p>
                @endif
            </div>
            <button type="button" wire:click="{{ $closeMethod }}" data-drawer-close class="rounded-md p-1.5 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700" aria-label="Close">
