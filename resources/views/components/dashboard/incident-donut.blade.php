...
@php
    ...
    $topLabel = $breakdown->keys()->first();
    $topShare = $total > 0 && $topLabel ? round(($breakdown->first() / $total) * 100, 1) : null;
@endphp
...
<section class="card-surface overflow-hidden">
    ...
    <div class="space-y-4 p-5">
        <x-dashboard.chart-metric
            :value="$total"
            label="Incidents"
            :hint="$topLabel && $topShare !== null ? $topLabel.' · '.$topShare.'% of total' : 'No categories yet'"
        />
                ...
                <div class="min-w-0 flex-1 space-y-0">
                    @foreach ($breakdown as $label => $count)
                        <div class="flex items-center justify-between border-b border-zinc-100 py-2 text-sm last:border-0 dark:border-zinc-800">
                            ...
                            <span class="shrink-0 font-semibold tabular-nums text-zinc-900 dark:text-zinc-100">{{ $count }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
