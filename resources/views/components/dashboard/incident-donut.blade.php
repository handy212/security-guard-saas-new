        ...
        <div class="flex flex-col items-center gap-5 sm:flex-row">
            ...
            <div class="min-w-0 flex-1 space-y-0">
                @foreach ($breakdown as $label => $count)
                    <div class="flex items-center justify-between border-b border-zinc-100 py-2 text-sm last:border-0">
                        <div class="flex min-w-0 items-center gap-2">
                            <span class="h-2.5 w-2.5 shrink-0 rounded-full" style="background: {{ $colors[$loop->index % count($colors)] }}"></span>
                            <span class="truncate text-zinc-600">{{ $label }}</span>
                        </div>
                        <span class="shrink-0 font-semibold text-zinc-900">{{ $count }}</span>
                    </div>
                @endforeach
            </div>
        </div>
